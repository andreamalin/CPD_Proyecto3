...
__global__ void GPU_HoughTranShared (unsigned char *pic, int w, int h, int *acc, float rMax, float rScale)
{
  ...
  */
  ...
  /*
    Se inicializan los valores de la matriz localAcc en cero.
    Cada hilo se encarga de esto.
  */
  ...
  */
  ...
  /*
  * Se asegura la inicializacion de la matriz antes de continuar
  */
  __syncthreads();
  ...
  */
  ...
  /**
   * Se asegura que hayan terminado de actualizar localAcc
   * antes de actualizar acc
  */
  __syncthreads();
  for (int i = localID; i < sharedSize; i += blockDim.x) 
  {
